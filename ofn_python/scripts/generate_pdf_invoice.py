...
def run():
    ...
    sheet_df, sheet = get_data_from_google_sheet(
        credentials_file, "Bauernbox Übersicht", ["number", "invoice_no"], "münster"
    )
    ...
    if not sheet_df.empty:
        for i in sheet_df["number"].tolist():
            ...
            doc.build(
                pdf_invoice.body, onFirstPage=add_bank_info, onLaterPages=add_bank_info
            )
            ...
            with ftplib.FTP(
                ...
                ftp.storbinary(
                    f"STOR invoices/invoice{invoice_no}.pdf", io.BytesIO(result)
                )

            cell_rows.append(worksheet.find(i).row)
            new_data.append(
                [invoice_no, dt.datetime.now().strftime("%Y-%m-%dT%H:%M:%S")]
            )
