...
CREDENTIALS_FILE = (
    f'{os.environ["PATH_TO_OFN_PYTHON"]}/creds/openfoodnetwork-294510-db0f5d386f9f.json'
)
...
def get_and_write_new_items(data, sheet_df, worksheet):
    ...
    for product in data["products"]:
        temp = pd.json_normalize(product["variants"])[
            [
                "producer_name",
                "sku",
                "name",
    ...
    variants = variants[~variants["sku"].isin(sheet_df["sku"].tolist())].reset_index(
        drop=True
    )
    ...
    if not variants.empty:
        ...
        variants["unit_to_display"] = variants["unit_to_display"].apply(
